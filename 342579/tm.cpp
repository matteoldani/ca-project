 ...
 */
struct transaction {
    std::map<uintptr_t, void *>  ws_map; // hashmap containing the write set
    std::unordered_set<void *>   rs_map; // hashmap containing the read set
...
struct region {
    ...
    */
    ...
    size_t                  size;
    std::atomic_uint64_t    global_lock;    // global version lock updated with a compare and swap
...
void release_write_set_locks(struct region *region, struct word_lock * last_word_lock){
    ...
    for(const auto &wsn: transaction.ws_map){
        ...
        get_word_lock((void*)wsn.first, region, &word_lock);
...
tx_t tm_begin(shared_t unused(shared), bool is_ro) noexcept{
    ...
    transaction.rv = region->global_lock.load();
...
bool validate_read_set(struct region *region){
    ...
    for(const auto rsn: transaction.rs_map){
        ...
        if(transaction.ws_map.find((uintptr_t)rsn) != transaction.ws_map.end()){
            is_in_ws_map = true;
        }
...
bool tm_end(shared_t shared, tx_t unused(tx)) noexcept{
    ...
    if(transaction.is_ro || transaction.ws_map.empty()){
        free_transaction();
        return true;
    }
...
bool tm_read(shared_t shared, tx_t unused(tx), void const* source, size_t size, void* target) noexcept{
    ...
    for(uintptr_t i=(uintptr_t)source; i<(uintptr_t)source + size; i=i+region->align){
        ...
        if(!transaction.is_ro){
            ...
            transaction.rs_map.emplace(addr_i);
        } 
...
bool tm_write(shared_t shared, tx_t unused(tx), void const* source, size_t size, void* target) noexcept{
    ...
    for(uintptr_t i=(uintptr_t)target; i<(uintptr_t)target + size; i=i+align){
        ...
        void *value = (malloc(sizeof(align)));
        memcpy(value, (void*)((uintptr_t)source+(i-(uintptr_t)target)), align);
        transaction.ws_map[i] = value;
